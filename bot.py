...
async def removes4s(interaction: discord.Interaction):
    ...
    if result == "DELETE 0":
        embed = discord.Embed(title="❌ Not an S4S Channel", description=f"{interaction.channel.mention} is not a registered S4S channel.", color=discord.Color.red())
    else:
        embed = discord.Embed(title="✅ S4S Channel Removed", description=f"{interaction.channel.mention} is no longer monitored.", color=discord.Color.orange())
    await interaction.response.send_message(embed=embed)
...
async def resets4swarns(interaction: discord.Interaction, user: discord.Member):
    ...
    embed = discord.Embed(title="✅ Warnings Reset", description=f"{user.mention} warnings reset to **0/3**.", color=discord.Color.green())
    await interaction.response.send_message(embed=embed)
...
async def s4sreset(interaction: discord.Interaction, user: discord.Member):
    ...
    embed = discord.Embed(title="✅ Cooldown Reset", description=f"{user.mention} can ping immediately.", color=discord.Color.green())
    await interaction.response.send_message(embed=embed)
...
async def s4sinfo(interaction: discord.Interaction):
    ...
    channels = "\n".join(f"<#{r['channel_id']}>" for r in rows) if rows else "None — use `/sets4s` in a channel."
